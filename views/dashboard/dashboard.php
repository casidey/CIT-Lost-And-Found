...
try {
    $stmt3 = $pdo->prepare("
        SELECT v.*, r.title AS item_title, r.verification_question, r.verification_answer,
        ...
        WHERE r.user_id = ? AND v.status = 'pending'
...
// ── Show success toast if redirected from report form ──────────────────────
...
<head>
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    colors: { citred: '#DC2626', citdarkred: '#b91c1c' }
                }
            }
        }
    </script>
            ...
            <?php if (!empty($verification_requests)): ?>
                ...
                <div class="bg-[#EEF2F6] rounded-xl border border-[#D0D7E2] p-6 mb-8 shadow-sm">
                    <div class="flex items-center gap-3 mb-5">
                        ...
                        <div>
                            <h3 class="font-bold text-gray-900 text-lg leading-tight">Verification Request</h3>
                    ...
                    <div class="flex flex-col md:flex-row justify-between md:items-center gap-5">
                            ...
                            <div class="bg-white border border-gray-200 rounded-lg p-4 inline-block w-full md:w-auto shadow-sm">
                                ...
                                <div class="flex gap-10">
                                    <div>
                                        ...
                                        <span class="bg-green-50 text-green-700 text-[12px] font-bold px-3 py-1 rounded-md"><?= htmlspecialchars($vr['verification_answer']) ?></span>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-gray-500 tracking-wider uppercase mb-1.5">Their Answer</p>
                                        <span class="bg-blue-50 text-blue-700 text-[12px] font-bold px-3 py-1 rounded-md"><?= htmlspecialchars($vr['claimant_answer']) ?></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="flex items-center gap-3">
                            ...
                            <a href="index.php?page=verification&action=approve&id=<?= (int)$vr['id'] ?>"
                               class="bg-[#60A06D] hover:bg-[#4E885A] text-white font-bold text-sm px-6 py-2.5 rounded-lg flex items-center gap-2 transition-all shadow-sm">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between p-4 hover:bg-gray-50 rounded-xl transition cursor-pointer group">
                        <div class="flex items-center gap-4">
                            <div class="w-10 h-10 <?= $type_color ?> text-white font-bold rounded-lg flex items-center justify-center shadow-sm text-sm">
                                <?= $type_char ?>
                            </div>
                            <div>
                        ...
                        <div class="flex items-center gap-4">
                            <span class="text-[10px] font-bold px-3 py-1 rounded-md <?= $status_color ?> uppercase tracking-tighter">
                                <?= $status_label ?>
                            </span>
